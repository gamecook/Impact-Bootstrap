...
ig.module(
    'game.bootstrap.entities.weapons'
)
    .requires(
    'impact.entity',
    'impact.sound',
    ...
    .defines(function ()
    {
        ...
        EntityBullet = EntityWeapons.extend({
            ...
            maxPool:2,
            gunFireSFX:new ig.Sound('media/sounds/GunFire.*'),
        ...
        EntityShotgunShell = EntityWeapons.extend({
            ...
            distance:20,
            maxPool:1,
            shotgunFireSFX:new ig.Sound('media/sounds/ShotgunFire.*'),
            init:function (x, y, settings)
            {
                this.parent(x + (settings.flip ? -4 : 8), y + 8, settings);
                this.vel.x = this.accel.x = (settings.flip ? -this.maxVel.x : this.maxVel.x);
        ...
        EntityMachineGun = EntityWeapons.extend({
            ...
            recoil:5,
            maxPool:4,
            machineGunFireSFX:new ig.Sound('media/sounds/MachineGunFire.*'),
            init:function (x, y, settings)
            {
        ...
        EntityGrenade = EntityWeapons.extend({
            ...
            blastRadius:30,
            maxPool:1,
            granadeBounceSFX:new ig.Sound('media/sounds/GranedeBounce.*'),
            granadeExplodeSFX:new ig.Sound('media/sounds/GrenedeExplosion.*'),
            init:function (x, y, settings)
            {
        ...
        EntityMine = EntityWeapons.extend({
            size:{x:6, y:5},
            offset:{x:0, y:0},
            animSheet:new ig.AnimationSheet('media/mine.png', 6, 5),
            type:ig.Entity.TYPE.A,
            checkAgainst:ig.Entity.TYPE.B,
            collides:ig.Entity.COLLIDES.ACTIVE,
            maxVel:{x:50, y:100},
            ...
            friction:{x:100, y:0},
            maxPool:1,
            mineBeepSFX:new ig.Sound('media/sounds/MineBeep.*'),
            mineExplosionSFX:new ig.Sound('media/sounds/MineExplosion.*'),
            init:function (x, y, settings)
            {
